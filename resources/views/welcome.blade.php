...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body class="min-h-screen bg-slate-950 font-sans text-slate-100 antialiased">
        <main class="relative isolate min-h-screen overflow-hidden">
            <div class="absolute inset-x-0 top-0 -z-10 h-96 bg-gradient-to-br from-[#0097B2]/20 via-slate-950 to-slate-950"></div>

            <div class="mx-auto flex min-h-screen max-w-7xl flex-col px-6 py-8 sm:px-10 lg:px-12">
                <header class="flex items-center justify-between">
                    <a href="{{ route('home') }}" class="flex items-center gap-3" wire:navigate>
                        <x-application-logo class="h-10 w-10 fill-current text-[#0097B2]" />
                    ...
                    <nav class="flex items-center gap-2 text-sm">
                        ...
                        @else
                            ...
                            @if (Route::has('register'))
                                <a href="{{ route('register') }}" class="rounded-md bg-[#0097B2] px-4 py-2 font-semibold text-white transition hover:bg-[#007A91]" wire:navigate>{{ __('Register') }}</a>
                            @endif
                        @endauth
                    </nav>
                </header>
                ...
                <section class="flex flex-1 items-center py-20 lg:py-28">
                    <div class="max-w-3xl">
                        <p class="text-sm font-semibold uppercase tracking-[0.2em] text-[#7ED957]">{{ __('Personal finance, made clear') }}</p>
                        ...
                        <div class="mt-10 flex flex-wrap gap-3">
                            @auth
                                <a href="{{ route('dashboard') }}" class="rounded-md bg-[#0097B2] px-5 py-3 font-semibold text-white transition hover:bg-[#007A91]" wire:navigate>{{ __('Open dashboard') }}</a>
                            @else
                                <a href="{{ route('register') }}" class="rounded-md bg-[#0097B2] px-5 py-3 font-semibold text-white transition hover:bg-[#007A91]" wire:navigate>{{ __('Get started') }}</a>
